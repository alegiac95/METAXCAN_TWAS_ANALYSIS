...
OUTPUT_DIR="/Users/alessiogiacomel/Dropbox/PhD/Analysis/transcriptomics_gio/MetaXcan_TWAS_Analysis/results/AN"
METAXCAN="/Users/alessiogiacomel/MetaXcan/software"
MODELS_DIR="${DATA_DIR}/models/elastic_net_models"
GWAS_FILE="${DATA_DIR}/GWAS/AN/pgcAN2.2019-07.vcf.tsv.gz"
...
for model in ${MODELS_DIR}/eqtl/*.db; do
    ...
    if [ ! -f $OUTPUT_DIR/en_PGC_AN_${model_name}.csv ]; then
        ...
        python $METAXCAN/SPrediXcan.py \
            ...
            --skip_until_header CHROM \
            --keep_non_rsid \
            --additional_output \
            --output_file $OUTPUT_DIR/en_PGC_AN_${model_name}.csv
...
python $METAXCAN/SMultiXcan.py \
    ...
    --metaxcan_file_name_parse_pattern "(.*)_PGC_AN_en_(.*).csv" \
    --gwas_file $GWAS_FILE \
    --snp_column ID \
    --chromosome_column CHROM \
    --position_column POS \
    ...
    --throw \
    --verbosity 3 \
    --output $OUTPUT_DIR/PGC_AN_SMetaXcan_en.tsv