...
OUTPUT_DIR="/Users/alessiogiacomel/Dropbox/PhD/Analysis/transcriptomics_gio/MetaXcan_TWAS_Analysis/results/MDD"
METAXCAN="/Users/alessiogiacomel/MetaXcan/software"
MODELS_DIR="${DATA_DIR}/models/elastic_net_models"
GWAS_FILE="${DATA_DIR}/GWAS/MDD/PGC_UKB_depression_genome-wide_updated.txt"
...
for model in ${MODELS_DIR}/eqtl/*.db; do
    ...
    if [ ! -f $OUTPUT_DIR/en_PGC_MDD_${model_name}.csv ]; then
        ...
        python $METAXCAN/SPrediXcan.py \
            ...
            --keep_non_rsid \
            --additional_output \
            --output_file $OUTPUT_DIR/en_PGC_MDD_${model_name}.csv
...
python $METAXCAN/SMultiXcan.py \
    ...
    --metaxcan_file_name_parse_pattern "(.*)_PGC_MDD_en_(.*).csv" \
    --gwas_file $GWAS_FILE \
    --snp_column MarkerName \
    --effect_allele_column A1 \
    --non_effect_allele_column A2 \
    ...
    --throw \
    --verbosity 3 \
    --output $OUTPUT_DIR/PGC_MDD_SMetaXcan_en.tsv