...
OUTPUT_DIR="/Users/alessiogiacomel/Dropbox/PhD/Analysis/transcriptomics_gio/MetaXcan_TWAS_Analysis/results/SCZ"
METAXCAN="/Users/alessiogiacomel/MetaXcan/software"
MODELS_DIR="${DATA_DIR}/models/elastic_net_models"
GWAS_FILE="${DATA_DIR}/GWAS/SCZ/daner_natgen_pgc_eur.gz"
...
for model in ${MODELS_DIR}/eqtl/*.db; do
    ...
    if [ ! -f $OUTPUT_DIR/en_PGC_SCZ_${model_name}.csv ]; then
        ...
        python $METAXCAN/SPrediXcan.py \
            ...
            --keep_non_rsid \
            --additional_output \
            --output_file $OUTPUT_DIR/en_PGC_SCZ_${model_name}.csv
...
python $METAXCAN/SMultiXcan.py \
    ...
    --metaxcan_file_name_parse_pattern "(.*)_PGC_SCZ_en_(.*).csv" \
    --gwas_file $GWAS_FILE \
    --snp_column SNP \
    --chromosome_column CHR \
    --position_column BP \
    ...
    --throw \
    --verbosity 3 \
    --output $OUTPUT_DIR/PGC_SCZ_SMetaXcan_en.csv