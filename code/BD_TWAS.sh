...
OUTPUT_DIR="/Users/alessiogiacomel/Dropbox/PhD/Analysis/transcriptomics_gio/MetaXcan_TWAS_Analysis/results/BD"
METAXCAN="/Users/alessiogiacomel/MetaXcan/software"
MODELS_DIR="${DATA_DIR}/models/elastic_net_models"
GWAS_FILE="${DATA_DIR}/GWAS/BD/daner_PGC_BIP32b_mds7a_0416a.gz"
...
for model in ${MODELS_DIR}/eqtl/*.db; do
    ...
    if [ ! -f $OUTPUT_DIR/en_PGC_BD_${model_name}.csv ]; then
        ...
        python $METAXCAN/SPrediXcan.py \
            ...
            --keep_non_rsid \
            --additional_output \
            --output_file $OUTPUT_DIR/en_PGC_BD_${model_name}.csv
...
python $METAXCAN/SMultiXcan.py \
    ...
    --metaxcan_file_name_parse_pattern "(.*)_PGC_BD_en_(.*).csv" \
    --gwas_file $GWAS_FILE \
    --snp_column SNP \
    --chromosome_column CHR \
    --position_column BP \
    ...
    --throw \
    --verbosity 3 \
    --output $OUTPUT_DIR/PGC_BD_SMetaXcan_en.tsv